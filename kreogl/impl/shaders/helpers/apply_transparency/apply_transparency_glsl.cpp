#include "apply_transparency_glsl.hpp"

namespace kreogl {
	const char * apply_transparency_glsl::frag = R"(
#version 330

void apply_transparency(float alpha) {
	const int matrix_side = 4;
	const float threshold_matrix[matrix_side * matrix_side] = float[matrix_side * matrix_side](
		1.0,  9.0,  3.0, 11.0,
		13.0,  5.0, 15.0,  7.0,
		4.0, 12.0,  2.0, 10.0,
		16.0,  8.0, 14.0,  6.0
	...
	if (alpha < threshold_matrix[(x % matrix_side) * matrix_side + y % matrix_side] / (matrix_side * matrix_side + 1))
		discard;
}
)";
}