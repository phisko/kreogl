...
namespace kreogl {
	const char * spot_light_glsl::frag = R"(
#version 330
...
vec3 calc_spot_light(vec3 world_pos, vec3 normal, vec3 diffuse, vec3 specular) {
...
void main() {
	...
	output_color = vec4(calc_spot_light(world_pos, normal, diffuse, specular), 1.0);
})";
}