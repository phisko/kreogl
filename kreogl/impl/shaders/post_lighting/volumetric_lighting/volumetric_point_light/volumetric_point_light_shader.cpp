#include "volumetric_point_light_shader.hpp"
...
namespace kreogl {
	volumetric_point_light_shader::volumetric_point_light_shader() noexcept {
		KREOGL_PROFILING_SCOPE;

		init("volumetric_point_light_shader");
...
#ifndef NDEBUG
		assert(_shadow_cube_glsl.view_pos.location == _glsl.view_pos.location);
		_shadow_cube_glsl.view_pos.used = true;

		assert(_shadow_cube_glsl.position.location == _point_light_glsl.position.location);
		_shadow_cube_glsl.position.used = true;
#endif
	}

	void volumetric_point_light_shader::add_source_files() noexcept {
		KREOGL_PROFILING_SCOPE;
	...
	std::vector<uniform_base *> volumetric_point_light_shader::get_uniforms() noexcept {
		KREOGL_PROFILING_SCOPE;
		...
		return {
			...
			&_glsl.screen_size,
			&_glsl.color,
			// point_light_glsl::get_direction
	...
	void volumetric_point_light_shader::draw(const draw_params & params) noexcept {
		KREOGL_PROFILING_SCOPE;
